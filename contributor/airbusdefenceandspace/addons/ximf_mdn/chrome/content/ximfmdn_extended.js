 ...
 *   Copyright (c) 2014 CASSIDIAN - All rights reserved
 *
...
var MDN_DISPOSE_TYPE_DELETED = 3;
var MDN_Registered=false;
//Save the original built SetupCommandUpdateHandlers functions
var mdn_extended_OriginalSetupCommandUpdateHandlers;
if(!mdn_extended_OriginalSetupCommandUpdateHandlers)
{
...
var mdn_extended_MDN_DeleteController = {
    ...
    doCommand: function(command) {
        ...
        var selectedMessages = gFolderDisplay.selectedMessageUris;
        if (selectedMessages) {
...
function mdn_extended_HandleMDNDeleteResponse(aUrl, headers) {
    ...
    var msgURI = gFolderDisplay.selectedMessageUris[0]; //GetLoadedMessage();
    if (!msgFolder || !msgURI || gFolderDisplay.selectedMessageIsNews) {
        return;
    }
   ...
   if (msgFolder.isSpecialFolder( Components.interfaces.nsMsgFolderFlags.Trash, true)) {
    ...
    // Check if the positive MDN message has been sent
    if ((msgFlags & Components.interfaces.nsMsgMessageFlags.MDNReportSent)) {
        return;
    }
    ...
    if (mdnGenerator.process(MDN_DISPOSE_TYPE_DELETED, msgWindow, msgFolder, msgHdr.messageKey, mimeHdr, true)) {
...
SetupCommandUpdateHandlers = function mdn_extended_SetupCommandUpdateHandlers() {
	//Test if the MDN controller is registered
	// to have not some calls in loop
	if(!MDN_Registered)
	{
