...
function ReadXmlHeadersToSecure(params){
	try{
		...
		if(!file.exists()){
			gConsole.logStringMessage("[msgCompSecurityInfo.js - ReadXmlHeadersToSecure] Error loading schema file : " + completePath);
			return;
		}	
		...
		if(compatibleTag.length>0){
			...
			gConsole.logStringMessage("[msgCompSecurityInfo.js - ReadXmlHeadersToSecure] no headers tag in file " + completePath);
		}	
		} catch (e) {
