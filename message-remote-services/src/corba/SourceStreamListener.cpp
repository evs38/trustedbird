...
NS_IMPL_ISUPPORTS1(SourceStreamListener,nsIStreamListener)

nsresult SourceStreamListener::OnDataAvailable(nsIRequest *aRequest,
...
nsresult SourceStreamListener::OnStartRequest(nsIRequest* aRequest, nsISupports* aContext) {
	m_done = PR_FALSE;
	return NS_OK;
}

nsresult SourceStreamListener::OnStopRequest(nsIRequest* aRequest, nsISupports* aContext, nsresult rv) {
	m_done = PR_TRUE;
	return NS_OK;
}
