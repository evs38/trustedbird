...
public class MessageHandlersHolder {
	...
	public MessageHandlersHolder() {
		holder = new CMessageHdrsHolder();
	}

	public MessageHandlersHolder(CMessageHdrsHolder holder) {
		this.holder = holder;
	}
	
	public MessageHandler[] getValue(){
		return convert(holder);
	}
	
	private MessageHandler[] convert(CMessageHdrsHolder cHolder) {
		CMessageHdr[] cHdr = cHolder.value;
		MessageHandler[] handlers = new MessageHandler[cHdr.length];
		
		for (int i = 0; i < cHdr.length; i++) {
			handlers[i] = new MessageHandler(cHdr[i]);
		}
		
		return handlers;
	}

	public CMessageHdrsHolder getCorbaObject(){
		return holder;
	}
