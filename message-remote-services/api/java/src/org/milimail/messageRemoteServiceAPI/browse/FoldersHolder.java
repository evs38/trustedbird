...
public class FoldersHolder {
	...
	public FoldersHolder() {
		holder = new CFoldersHolder();
	}
	
	public Folder[] getValue(){
		CFolder[] cFolders = holder.value;
		Folder[] folders = convert(cFolders);
		return folders;
	}

	private Folder[] convert(CFolder[] cFolders) {
		Folder[] folders = new Folder[cFolders.length];
		for (int i = 0; i < cFolders.length; i++) {
			folders[i] = new Folder(cFolders[i]);
		}
		
		return folders;
	}
	
	public CFoldersHolder getCorbaObject(){
		return holder;
	} 
}
