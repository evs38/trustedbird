...
// Proxy call descriptor class. Mangled signature:
//  void_i_cAccount_i_cCMessage_i_cMessageSendListener_e_cInternalServerException
...
#ifdef HAS_Cplusplus_catch_exception_by_base
  impl->SendMessage(*tcd->arg_0, *tcd->arg_1, tcd->arg_2);
#else
  if (!cd->is_upcall())
    impl->SendMessage(*tcd->arg_0, *tcd->arg_1, tcd->arg_2);
  else {
...
void _objref_MessageComposeService::SendMessage(const Account& p_account, const CMessage& p_message, MessageSendListener_ptr p_listener)
{
