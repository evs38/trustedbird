...
# if defined(SOLARIS_THREADS) || defined(WIN32_THREADS) \
     ...
     || defined(IRIX_PCR_THREADS) || defined(GENERIC_THREADS)
...
void GC_save_callers(struct callinfo info[NFRAMES]) 
{
	...
	currentSP = GetSP();				// GC_save_callers's frame.
	currentSP = *((void **)currentSP);	// GC_debug_malloc's frame.
	currentSP = *((void **)currentSP);	// GC_debug_malloc's caller's frame.
