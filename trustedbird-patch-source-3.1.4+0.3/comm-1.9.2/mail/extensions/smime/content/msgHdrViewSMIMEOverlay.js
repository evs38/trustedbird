...
{ 
  ...
  secureHeadersStatus: function(aSecureHeaders, aCanonAlgo)
  {
	...
	if(aSecureHeaders)
	{
		//gConsole.logStringMessage("secureHeadersStatus analysis - aCanonAlgo = " + aCanonAlgo);
		...
		for(var i=0;i<secureHeaders.length;++i)
		{
			...
			if(sHeader){
				...
				var headerMimeValue = "";	// value in the MIME message
				var valdecoded = "";		
				...
				if(hdrStatus == "0") //Verify the value validity only in the case where header status is duplicated
				{
					...
					for(var j=0;j<hdrArray.length;++j){
						...
						if(hdrName==hdrArray[j]._hdrName){
							...
							var charset = getMimeValueCharset(headerMimeValue);						
							valdecoded = _from_utf8(mimeParam.decodeRFC2047Header(hdrValue,charset,false,true)); //decode the value to display
							headerMimeValue = _from_utf8(mimeParam.decodeRFC2047Header(headerMimeValue,charset,false,true)); //decode the value to display
				...
				gSecureHeaders+=headerMimeValue+HEADER_VAL_SEPARATOR;
...
{
	// convert all header field nammes to lowercase
